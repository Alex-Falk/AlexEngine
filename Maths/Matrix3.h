...
namespace Maths
{
	...
	public:
		...
		//Default Accessors
		float operator[](int index) const { return mat_array[index]; }
		float& operator[](int index) { return mat_array[index]; }
		float operator()(int row, int col) const { return mat_array[row + col * 3]; }
		float& operator()(int row, int col) { return mat_array[row + col * 3]; }

		const Vector3& GetCol(int idx) const { return *((Vector3*)&mat_array[idx * 3]); }
		void SetCol(int idx, const Vector3& row) { memcpy(&mat_array[idx * 3], &row.x, 3 * sizeof(float)); }

		Vector3 GetRow(int idx) const { return Vector3(mat_array[idx], mat_array[3 + idx], mat_array[6 + idx]); }

		void SetRow(int idx, const Vector3& col)
		{
			mat_array[idx] = col.x;
		...
		Vector3 GetScalingVector() const { return Vector3(_11, _22, _33); }
		void SetScalingVector(const Vector3& in) { _11 = in.x, _22 = in.y, _33 = in.z; }
	...
	Matrix3& operator+=(Matrix3& a, float b);
	Matrix3& operator-=(Matrix3& a, float b);
	Matrix3& operator*=(Matrix3& a, float b);
	Matrix3& operator/=(Matrix3& a, float b);
