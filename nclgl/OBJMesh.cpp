...
bool OBJMesh::LoadOBJMesh(std::string filename)
{
	...
	*/
	...
	auto currentMesh = new OBJSubMesh();
	...
	while (!f.eof())
	{
		...
		else if (currentLine == MTLDIFFUSEMAP)
		{
			...
			if (!currentMTL.diffuse.empty())
			{
				...
				currentMTL.diffuseNum = SOIL_load_OGL_texture(filename.c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID,
		...
		else if (currentLine == MTLBUMPMAP || currentLine == MTLBUMPMAPALT)
		{
			...
			if (!currentMTL.bump.empty())
			{
				...
				NCLDebug::Log("    -> Loading Texture: %s", filename.c_str());
				currentMTL.bumpNum = SOIL_load_OGL_texture(filename.c_str(), SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID,
				                                           SOIL_FLAG_INVERT_Y | SOIL_FLAG_TEXTURE_REPEATS);
			}
		}
	}
...
void OBJMesh::FixTextures(MTLInfo& info)
{
	if (!info.bumpNum)
	{
		...
		info.bumpNum = SOIL_load_OGL_texture(string(Graphics::TextureDir + info.bump).c_str(), SOIL_LOAD_AUTO,
