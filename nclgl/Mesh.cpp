...
void Mesh::BufferData()
{
	...
	if (tangents)
	{
		...
		glVertexAttribPointer(TANGENT_BUFFER, 3, GL_FLOAT, GL_FALSE, 0, nullptr);
...
void Mesh::GenerateTangents()
{
	...
	if (indices)
	{
		for (GLuint i = 0; i < numIndices; i += 3)
		{
			...
			Vector3 tangent = GenerateTangent(vertices[a], vertices[b], vertices[c], textureCoords[a], textureCoords[b],
			                                  textureCoords[c]);
                              ...
                              const Vector2& tb, const Vector2& tc)
{
	...
	auto axis2 = Vector3(vertex1 - vertex2);

	auto axis = Vector3(vertex1 * coord2.y - vertex2 * coord1.y);
...
void Mesh::DrawDebugNormals(float length)
{
	if (numVertices > 0)
	{
		...
		auto tempV = new Vector3[numVertices * 2];
		auto tempC = new Vector3[numVertices * 2];
		...
		glVertexAttribPointer(COLOUR_BUFFER, 3, GL_FLOAT, GL_FALSE, 0, nullptr);
		glEnableVertexAttribArray(COLOUR_BUFFER);
